...
| Images | `public/images/` + `src/data/palia.ts` | Own licensed / provided assets only |
...
4. Replace images; update **`palia.ts`** (image registry) paths/alts
...
- **UI brand strings**: Hero, footer, 404, reviews CTAs, blog eyebrow read from `siteConfig` / `forumLabel` — do not hardcode.
