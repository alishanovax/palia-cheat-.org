/**
 * SVG overlay generators for Palia cheats marketing images.
 * Composited on top of IGN Palia gameplay screenshots.
 */
...
export function hacksBadge(w, h) {
	return `
		...
		<text x="${w - 160}" y="${h - 18}" fill="${PURPLE}" font-family="Segoe UI,Arial,sans-serif" font-size="12" font-weight="700">PALIA CHEATS</text>
	`;
}
...
export function buildOverlaySvg(width, height, preset) {
	...
	switch (preset) {
		...
		case 'loot':
			...
			body += espBox(width * 0.68, height * 0.52, width * 0.06, height * 0.08, 'fishing timer · 103m', '#fbbf24');
