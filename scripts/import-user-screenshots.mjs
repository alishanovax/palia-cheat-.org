...
const assetsDir =
	'C:/Users/Aman/.cursor/projects/c-Users-Aman-paliahacks/assets';
...
const USER_SCREENSHOTS = [
	{
		...
		file: 'palia-esp-resource-tags.webp',
	},
	{
		source:
		...
		file: 'palia-wallhack-overlay.webp',
	},
	{
		source:
		...
		file: 'palia-aimbot-hunting.webp',
	},
	{
		source:
		...
		file: 'palia-aimbot-overlay.webp',
	},
	{
		source:
	...
	},
	{
		...
		file: 'palia-cheats-gameplay.webp',
	},
];
...
const REMOVE_PREFIXES = [
	'palia-battle-royale-combat',
	'palia-extract-fight',
	'palia-cheats-aimbot',
	'palia-cheats-cover',
	'palia-cheats-esp-wallhack',
	'palia-cheats-hero',
	'palia-cheats-logo',
	'palia-cheats-package',
	'palia-header-art',
	'palia-loadout-builder',
	'palia-player-esp',
	'palia-scav-run-combat',
	'palia-scav-run-mode',
	'palia-squad-fight',
	'palia-verdansk-map',
];

async function removeOldPaliaImages() {
	const files = await readdir(imagesDir).catch(() => []);
	for (const file of files) {
		if (file.includes('palia-cheats-logo')) continue;
...
async function generateResponsiveVariants() {
	const heroSource = path.join(imagesDir, 'palia-esp-resource-tags.webp');
	const heroMeta = await sharp(heroSource).metadata();

	for (const width of HERO_WIDTHS) {
		if (heroMeta.width && width > heroMeta.width) continue;
		const file = `palia-esp-resource-tags-${width}w.webp`;
	...
	for (const { file } of USER_SCREENSHOTS) {
		if (file === 'palia-esp-resource-tags.webp') continue;
...
async function generateFavicons() {
	const logoPath = path.join(imagesDir, 'palia-cheats-logo.png');
