...
const imageMap: Record<BlogImageKey, string> = {
	hero: paliaImages.espOverlay,
	espWallhack: paliaImages.cheatsWallhack,
	aimbotCombat: paliaImages.cheatsAimbot,
	aimbotSkeleton: paliaImages.cheatsMenu,
	squadFight: paliaImages.cheatsAimbot,
	headerArt: paliaImages.cheatsMenu,
	cheatsPackage: paliaImages.cheatsGarden,
	playerEsp: paliaImages.cheatsEsp,
	rebootFight: paliaImages.cheatsTeleport,
	battleRoyaleCombat: paliaImages.cheatsEsp,
	battleRoyaleIslandMap: paliaImages.hero,
};

const FALLBACK_BLOG_IMAGE = paliaImages.espWallhack;
...
export function isForumPath(pathname: string): boolean {
	...
	return Boolean(context.isForumIndex || context.forumSlug);
}
...
/** Target URL for the same forum index or post in another locale. */
export function getForumLocaleSwitchHref(pathname: string, targetLocale: LocaleCode): string {
	const context = resolvePageContextFromPath(pathname);

	if (context.forumSlug) {
		const post = findPostBySlug(context.forumSlug, context.locale) ?? findPostBySlug(context.forumSlug);
		if (post) {
			const slug = post.translations[targetLocale]?.slug ?? post.translations[defaultLocale].slug;
			return getForumPostPath(targetLocale, slug);
		}
	}
...
export function getForumPostPath(locale: LocaleCode, slug: string): string {
	const base = getForumBasePath(locale);
	return `${base}${slug}/`;
}

export function absoluteForumUrl(locale: LocaleCode, slug?: string): string {
	const path = slug ? getForumPostPath(locale, slug) : getForumBasePath(locale);
...
export function resolvePost(post: BlogPostDefinition, locale: LocaleCode): ResolvedBlogPost {
	...
	return {
		...
		canonicalPath: getForumPostPath(locale, translation.slug),
	};
}
...
export function getBlogPostHreflangAlternates(
	...
	const href = absoluteForumUrl(defaultLocale, post.translations[defaultLocale].slug);
	return [
...
export function getForumIndexHreflangAlternates(_currentLocale: LocaleCode = defaultLocale) {
	const href = absoluteForumUrl(defaultLocale);
	return [
		{ hreflang: locales.find((l) => l.code === defaultLocale)!.hreflang, href },
		{ hreflang: 'x-default' as const, href },
	];
 ...
 * Use EN `/forum/` only. Locale paths 301 to EN via [lang]/blog pages.
 */
...
export function getBlogSitemapEntriesForLocale(locale: LocaleCode) {
	...
	for (const post of blogPosts) {
		...
		const isProductPost = /Palia Cheats|Aimbot|ESP|Undetected|Comparisons/i.test(post.category);
		entries.push({
			path: getForumPostPath(locale, t.slug),
			lastmod: post.updated,
			priority: isProductPost ? 0.82 : 0.75,
			changefreq: 'weekly',
			images: [
				{
					url: new URL(imageSrc, siteConfig.url).href,
					title: t.title,
					caption: descriptiveAlt(imageSrc),
				},
			],
		});
	}
